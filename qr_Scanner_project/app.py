...
def send_to_google_sheets(data):
    try:
        payload = {
            "action": "submitQR",  # Ensure this matches what your Apps Script expects
            "data": data
        }
        headers = {"Content-Type": "application/json"}
        response = requests.post(GOOGLE_SHEET_URL, json=payload, headers=headers, timeout=5)

        # Log raw response content
        logging.info(f"Google Sheets Raw Response: {response.text}")

        # Check if response contains JSON
        try:
            response_json = response.json()
        except json.JSONDecodeError:
            logging.error("Google Sheets response is not in JSON format.")
            return False
